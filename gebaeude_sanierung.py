...
if st.button("Analyse starten"):
    ...
    if blattname is None:
        ...
        if df_passend.empty:
            ...
            # Maßnahmen sortieren nach Einsparung
            top_massnahmen = alle_massnahmen.sort_values(by="Einsparung_%", ascending=False).head(3)

            # Ausgabe
            st.success("Top 3 optimale Sanierungsmaßnahmen:")

            for idx, row in top_massnahmen.iterrows():
                st.markdown(f"""
                **Platz {top_massnahmen.index.get_loc(idx) + 1}:**
                - Maßnahme: **{row['Maßnahme']}**
                - Energieeinsparung: **{row['Einsparung_%']:.2f}%**
                """)
